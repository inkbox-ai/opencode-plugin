...
def _final_worker_text(task: Any) -> str:
    messages = _wire_worker_messages(task)
    if not messages:
        raise AssertionError("Completed A2A task returned no agent answer")
    return messages[-1]
...
def _inbound_single(a2a: Any, target: Any, timeout: float, run: str) -> None:
    ...
    try:
        ...
        if completion not in _final_worker_text(final):
...
def _inbound_multi(a2a: Any, target: Any, timeout: float, run: str) -> None:
    ...
    task = _send_task(
        a2a,
        target,
        "Please verify my access code, which I have not provided yet. "
        "Ask me for it and wait for my response before giving your final answer. "
        "After I reply, include both "
    ...
    try:
        ...
        history = _final_worker_text(final)
...
def _outbound_single(
    ...
    outer = _send_task(
        ...
        target,
        "Ask the agent at "
        f"{remote_card_url} to handle a task identified by "
        f"`{inner_token}`. Wait for that agent to finish before answering me, "
        "then include their response in your final answer.",
    )
    inner = None
    try:
        ...
        if worker_result not in _final_worker_text(final):
...
def _outbound_multi(
    ...
    outer = _send_task(
        ...
        "Ask the agent at "
        f"{remote_card_url} to handle a task identified by "
        f"`{inner_token}`. That agent will need a verification value; give them "
        f"`{answer}` when they ask. Wait for their finished result before "
        "answering me, then include their response in your final answer.",
    )
    inner = None
    try:
        inner = _find_inbound_task(remote_identity, inner_token, timeout)
